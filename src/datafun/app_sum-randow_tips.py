...
Author: Denise Case, Megan Randow
Date: 2026-06
...
uv run python -m datafun.app_sum-randow_tips
"""
...
def descriptive_stats(df_clean: pd.DataFrame) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    LOG.debug("Total Bill Statistics (using numpy):")
    LOG.debug(f"  Mean: {mean_total_bill:.2f}")
    LOG.debug(f"  Std Dev: {std_total_bill:.2f}")
    LOG.debug(f"  Min: {min_total_bill:.2f}")
    LOG.debug(f"  Max: {max_total_bill:.2f}")
    LOG.debug(f"  Range: {range_total_bill:.2f}")

    # Tip statistics
    mean_tip = np.mean(df_clean["tip"])
    std_tip = np.std(df_clean["tip"])
    min_tip = np.min(df_clean["tip"])
    max_tip = np.max(df_clean["tip"])
    range_tip = np.max(df_clean["tip"]) - np.min(df_clean["tip"])
    LOG.debug("Tip Statistics (using numpy):")
    LOG.debug(f"  Mean: {mean_tip:.2f}")
    LOG.debug(f"  Std Dev: {std_tip:.2f}")
    LOG.debug(f"  Min: {min_tip:.2f}")
    LOG.debug(f"  Max: {max_tip:.2f}")
    LOG.debug(f"  Range: {range_tip:.2f}")
...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    plt.savefig("docs/images/Figure_4.png")
    ...
    LOG.info("---- Creating Second Scatter Plot to see Relationships ------")
    LOG.info("----   Use clean dataframe ---------------------------")
    LOG.info("----   Set x to total bill ---------------------------")
    LOG.info("----   Set y to tip ----------------------------------")
    LOG.info("----   Set the hue (color mapping) to smoker column --")
    ...
    scatter_plt.set_xlabel("Total Bill ($)")
    scatter_plt.set_ylabel("Tip ($)")
    scatter_plt.set_title("Chart 2. Total Bill vs Tip (by Smoker Status)")
    plt.savefig("docs/images/Figure_5.png")
    ...
    box_plt.set_title("Chart 3. Tip by Day")
    plt.savefig("docs/images/Figure_6.png")
...
def summarize(df: pd.DataFrame, df_clean: pd.DataFrame) -> None:
    ...
    # Get the unique values in the grouping column (e.g. day names)
    ...
    LOG.info("  total_bill and tip (~0.68)")
    LOG.info("Suggested next step: ")
    LOG.info("  Model tip ~ total_bill with linear regression")
