...
def make_plots(df_clean: pd.DataFrame) -> None:
    ...
    # Section 8.2 Python cell - use "Run All" so prior cells are executed first.

    LOG.info("---- Creating a Second Scatter Plot to see Relationships ------")
    LOG.info("----   Use clean dataframe ---------------------------")
    LOG.info("----   Set x to bill length -----------------------")
    LOG.info("----   Set y to body mass --------------------------")
    LOG.info("----   Set the hue (color mapping) to the group column --")
    ...
    scatter_plt.set_xlabel("Bill Length (mm)")
    scatter_plt.set_ylabel("Body Mass (g)")

    # Set the title using the Matplotlib Axes set_title() method
    scatter_plt.set_title("Chart 1.2 Penguin Bill Length vs. Body Mass (by Species)")
    plt.savefig("docs/images/Figure_2b.png")
